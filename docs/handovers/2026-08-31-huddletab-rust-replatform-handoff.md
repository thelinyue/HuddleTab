...
1. 按 `docs/deployment/release-verification.md` 在真实 iPhone Safari/Home Screen PWA 完成人工验收；在此之前不能创建 `v0.0.3` tag 或发布镜像。
2. 真机证据通过后再进行最终发布准备，创建 `v0.0.3` tag、触发严格语义版本 workflow，并验证 GHCR 镜像；当前不得执行。
