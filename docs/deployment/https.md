...
空数据库首次访问会显示 `/setup` 网页初始化表单，首个成功提交者将成为系统管理员。当前不提供 Setup Token 或 CLI 初始化入口；初始化请求由绑定 pre-auth Cookie 的 HMAC CSRF token 和认证限流保护。浏览器发送 Origin 或 Sec-Fetch-Site 时服务端会校验其同源值，缺失这些可选头不会绕过 CSRF 校验；这不能替代网络隔离。
