...
2. 检查 PostgreSQL 数据目录或 volume 的属主、权限和可读写状态。
3. 启动 Compose 并查看应用日志与健康检查：
...
4. 如果应用版本发生变化，容器入口会在启动 `server.js` 前自动执行已提交的数据库 migration。migration 失败时应用不会继续启动。
5. 通过登录、活动列表、附件读取和关键业务流程确认恢复结果。
