...
1. 确认备份对应的应用版本，并使用该版本的镜像；停止 HuddleTab 应用和 PostgreSQL 容器，按照宿主备份工具的流程成套恢复 Compose 文件、密钥、PostgreSQL 数据目录和应用数据目录。下一版支持从 `v0.0.30` 原地升级；更早版本的备份须先在匹配的旧镜像中恢复并升级到 `v0.0.30`，不能直接交给下一版启动。
