...
HuddleTab 是一个面向活动、成员、消费记录和结算的多人协作记账应用，当前正式版为 `0.0.7`，运行栈为 React/Vite 与 Rust/Axum。

## 当前源码运行

正式镜像为 `ghcr.io/thelinyue/huddletab:0.0.7`，对应 Git tag `v0.0.7`。该版本使用 Rust/Axum 运行栈。
...
services:
  ...
  app:
    ...
    image: ghcr.io/thelinyue/huddletab:0.0.7
    restart: unless-stopped
    depends_on:
      postgres:
...
自动化发布门禁流程见[最终 Release Verification](docs/deployment/release-verification.md)。本次 `0.0.7` 发布跳过真实 iPhone Safari/Home Screen 验收，属于已知发布例外。
