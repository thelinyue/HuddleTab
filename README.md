...
HuddleTab 是一个面向活动、成员、消费记录和结算的多人协作记账应用，当前正式版为 `0.0.10`，运行栈为 React/Vite 与 Rust/Axum。
...
services:
  ...
  app:
    ...
    image: ghcr.io/thelinyue/huddletab:0.0.10
    restart: unless-stopped
    depends_on:
      postgres:
...
自动化发布门禁流程见[最终 Release Verification](docs/deployment/release-verification.md)。本次 `0.0.10` 发布跳过真实 iPhone Safari/Home Screen 验收，属于已知发布例外。
