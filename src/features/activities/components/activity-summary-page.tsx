...
function toShareText(data: ActivitySummaryData) {
  const names = new Map(
    data.balances.map((item) => [item.memberId, item.displayName]),
  );
  return [
    data.activityName,
    ...
    ...(data.recommendations.length
      ? data.recommendations.map(
          (item) =>
            `${names.get(item.payerMemberId) ?? "成员"} 向 ${names.get(item.receiverMemberId) ?? "成员"} 支付 ${money(data.currency, item.amountMinor)}`,
        )
      : ["当前无需推荐转账。"]),
  ].join("\n");
}
...
export function ActivitySummaryPage({
  ...
  readonly data: ActivitySummaryData;
}) {
  ...
  return (
    <section className="py-5">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {data.memberCount} 人 · 总支出{" "}
        <MoneyAmount
          currency={data.currency}
          amountMinor={BigInt(data.totalExpenseMinor)}
          size="sm"
        />
      </p>
      {notice ? (
      ...
      <section className="mt-6" aria-labelledby="summary-balance">
        <h2 id="summary-balance" className="text-base font-semibold">
          余额
        </h2>
        <p className="mt-2 text-xl font-semibold">
          ...
          <MoneyAmount
            currency={data.currency}
            amountMinor={BigInt(data.currentUserBalanceMinor)}
            tone={moneyTone(data.currentUserBalanceMinor)}
            size="lg"
          />
        </p>
        {data.balances.length ? (
          ...
          <EmptyState
            ...
            title="暂无余额"
            description="添加消费后，这里会展示每位成员的净余额。"
          />
        )}
      </section>
      <section className="mt-6" aria-labelledby="summary-recommendation">
        <h2 id="summary-recommendation" className="text-base font-semibold">
          推荐结算
        </h2>
        {data.recommendations.length ? (
          <ul className="mt-3 divide-y border-y">
            {data.recommendations.map((item) => (
              <li
                key={`${item.payerMemberId}-${item.receiverMemberId}`}
                className="py-3 text-sm"
              >
                {names.get(item.payerMemberId) ?? "成员"} 向{" "}
                {names.get(item.receiverMemberId) ?? "成员"} 支付{" "}
                <MoneyAmount
                  currency={data.currency}
...
export function ActivitySummaryLoader({
  activityId,
}: {
  ...
  useEffect(() => {
    void fetch(`/api/activities/${encodeURIComponent(activityId)}/summary`, {
      cache: "no-store",
    })
      .then(async (response) => {
        ...
        if (!response.ok || !body.data)
          throw new Error(
            body.error?.message ?? "结算摘要加载失败，请稍后重试。",
          );
        return body.data;
      })
      .then(setData)
  ...
  if (!data)
    return <p className="py-8 text-muted-foreground">正在加载结算摘要…</p>;
