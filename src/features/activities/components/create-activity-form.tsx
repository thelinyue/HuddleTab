...
export function CreateActivityForm({ timeZone }: { readonly timeZone: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = (await response.json().catch(() => undefined)) as
      { data?: { id?: string }; error?: { message?: string } } | undefined;
    if (!response.ok || !body?.data?.id) {
      setError(body?.error?.message ?? "创建活动失败，请稍后重试。");
      return;
    }
    window.location.assign(
      new URL(`/activities/${body.data.id}`, window.location.origin).toString(),
    );
  }
  return (
...
function Field({
  ...
  return (
    <div className="grid gap-2">
      <Label htmlFor={`activity-${id}`}>{label}</Label>
      <Input
        id={`activity-${id}`}
        name={id}
        type={type}
