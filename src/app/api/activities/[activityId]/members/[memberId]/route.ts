...
export async function DELETE(request: Request, context: { params: Promise<{ activityId: string; memberId: string }> }) {
  const [{ requireSession, sessionUserId }, { sql }, { MemberService }, { MaintenanceMode }, { applicationErrorResponse }] = await Promise.all([
    import("@/server/auth/session"), import("@/server/db/client"), import("@/server/services/member-service"), import("@/server/maintenance/maintenance-mode"), import("@/server/http/application-error-response"),
  ]);
  try {
    ...
    await new MemberService(sql, usage).remove({ session: { user: { id: sessionUserId(session) } }, activityId: params.activityId, memberId: params.memberId });
